...
+ (NSArray *)getIndexPath:(NSUInteger)count beginIndex:(NSInteger)beginIndex inSection:(NSInteger)section {
    NSMutableArray *array = [NSMutableArray arrayWithCapacity:count];
    for (int i = 0; i < count; ++i) {
        NSIndexPath *indexPath = [NSIndexPath indexPathForRow:beginIndex + i inSection:section];
...
- (void)registerClass:(Class)cellClass {
    ...
    [self registerClass:cellClass forCellReuseIdentifier:[cellClass identifier]];
}

- (__kindof UITableViewCell *)dequeueReusableCellWithClass:(Class)cellClass forIndexPath:(NSIndexPath *)indexPath {
    return [self dequeueReusableCellWithIdentifier:[cellClass identifier] forIndexPath:indexPath];
}

- (__kindof UITableViewCell *)dequeueReusableCellWithClass:(Class)cellClass {

    return [self dequeueReusableCellWithIdentifier:[cellClass identifier]];
}

- (void)registerHeaderFooterClass:(Class)headerClass {
    [self registerClass:headerClass forHeaderFooterViewReuseIdentifier:[headerClass identifier]];
}

- (__kindof UITableViewHeaderFooterView *)dequeueReusableHeaderFooterViewWithClass:(Class)clazz {
    return [self dequeueReusableHeaderFooterViewWithIdentifier:[clazz identifier]];
}
